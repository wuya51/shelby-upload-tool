...
function UploadPage({ signAndSubmitTransaction, showMessage, solanaConnected: appSolanaConnected, solanaPublicKey: appSolanaPublicKey, solanaWallet }) {
  ...
  const solanaConnected = appSolanaConnected || (solanaStatus === "connected");
  const solanaPublicKey = appSolanaPublicKey || wallet?.account?.address;
  ...
  const confirmSolanaUpload = async () => {
    ...
    try {
      ...
      if (!currentUploadData) {
        ...
        currentUploadData = { fileData, uniqueBlobName, fileSize, expirationMicros };
      }
      let attempts = 0;
      ...
      try {
        ...
        if (errorMessage.includes('User rejected') || errorMessage.includes('Cancel') || errorMessage.includes('cancelled') || errorMessage.includes('rejected') || errorMessage.includes('CANCELED') || errorMessage.includes('USER_REJECTED')) {
    ...
    try {
      ...
      if (connected && account) {
        ...
        if (typeof currentAccount === 'string') {
          solanaKeyObject = { toBase58: () => currentAccount };
          ...
          if (typeof currentAccount.toBase58 === 'function') {
            ...
            solanaKeyObject = { toBase58: () => currentAccount.toString() };
          } else {
      ...
      if (walletType === 'solana') {
        ...
        try {
          ...
          if (errorMessage.includes('The specified blob was not found')) {
            setUploadStatus('Preparing upload...');
          } else if (errorMessage.includes('User rejected') || errorMessage.includes('Cancel') || errorMessage.includes('cancelled') || errorMessage.includes('rejected') || errorMessage.includes('CANCELED') || errorMessage.includes('USER_REJECTED')) {
            showMessage('Transaction cancelled by user', 'info');
            setUploadStep('upload');
          } else {
        ...
        let responseBody = '';
        try { responseBody = await startResponse.text(); } catch {}
        if (!startResponse.ok) {
  ...
  const getAccountBalance = async () => {
    ...
    try {
      ...
      setSolanaBalance('0');
      try {
        const aptAmount = await aptos.getAccountAPTAmount({ accountAddress: parsedAddress });
        setAptBalance(aptAmount.toString());
      } catch {}
      try {
        const coinsData = await aptos.getAccountCoinsData({ accountAddress: parsedAddress, options: { limit: 10 } });
        if (Array.isArray(coinsData)) {
          for (const coin of coinsData) {
            if (coin && coin.asset_type) {
              ...
              if (isShelbyUSD && coin.amount) {
                setShelbyUsdBalance(coin.amount.toString());
              }
            }
          }
        }
      ...
      try {
        const ownedTokens = await aptos.getAccountOwnedTokens({
          ...
          options: { tokenStandard: 'v2', limit: 10 }
        });
          ...
          for (const token of ownedTokens) {
            if (token && token.metadata) {
              if (token.metadata.symbol === 'SHELBY_USD' || token.metadata.name === 'ShelbyUSD') {
                if (token.amount) {
                  setShelbyUsdBalance(token.amount.toString());
                }
              }
            }
          }
        ...
        if (solanaConnected && solanaPublicKey) {
          const response = await fetch(`https://api.devnet.solana.com`, {
            ...
            body: JSON.stringify({
              ...
              id: 1,
              method: 'getBalance',
              params: [solanaPublicKey.toString()],
            }),
          });
          const data = await response.json();
          if (data && data.result && data.result.value) {
            setSolanaBalance(data.result.value.toString());
          }
        }
      } catch {}
    } catch {
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          {solanaConnected ? (
            <>
              <SolanaUploader
                currentStep={2}
                fundedStorageAddress={storageAccountAddress ? storageAccountAddress.toString() : null}
                onFirstBlobUploaded={onFirstBlobUploaded}
                      ...
                      <div className="space-y-3">
                        ...
                        {!connected && solanaConnected && (
                          <div className="flex items-center p-3 rounded-lg bg-white">
                            ...
                            <div className="flex justify-between items-center w-full">
                              <span className="text-sm font-medium text-gray-700">Storage Account Address</span>
                              <span
                                className="text-sm font-medium text-gray-800 cursor-pointer hover:text-blue-600 transition-colors"
                                onClick={() => {
                                  if (storageAccountAddress) {
                    ...
                    <div className="space-y-3">
                      {connected && account ? (
                        <>
                          ...
                          <a
                            href="https://docs.shelby.xyz/apis/faucet/aptos"
                            target="_blank"
                            rel="noopener noreferrer"
    ...
    <Router>
      <>
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
                  ...
                  {connected ? (
                    ...
                    >
                      <span className="text-sm font-mono">
                        {account?.address ? `${parseAddress(account.address).slice(0, 6)}...${parseAddress(account.address).slice(-4)}` : 'Invalid Address'}
                      </span>
                    ...
                    <div className="relative overflow-visible">
                      ...
                      {showConnectMenu && (
                        <div className="absolute right-0 top-full mt-2 w-64 bg-gray-800 rounded-lg shadow-xl border border-gray-700 md:w-56 z-50">
                          <div className="py-2">
          ...
          <main>
            <Routes>
              <Route path="/" element={<UploadPage signAndSubmitTransaction={signAndSubmitTransaction} showMessage={showMessage} solanaConnected={solanaConnected} solanaPublicKey={solanaPublicKey} solanaWallet={solanaWallet} />} />
